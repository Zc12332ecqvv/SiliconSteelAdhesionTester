...
namespace SiliconSteelAdhesionTester.Forms
{
    ...
    public sealed class ManualTaskForm : Form
    {
        ...
        {
            int count;
            if (!ValidateCount(_orientedCount.Text, out count))
            {
                ShowCountError("取向数量");
                return;
            }
            if (!ValidateCount(_nonOrientedCount.Text, out count))
            {
        ...
        {
            int count;
            return int.TryParse((value ?? string.Empty).Trim(), out count) ? count : (int?)null;
        }
