    ...
    public partial class DebugForm : Form
    {
        ...
        private void BuildStationDebugPanel()
        {
            ...
            for (var index = 0; index < 4; index++)
            {
                ...
                var continuous = NewStationButton("S" + station + " 连续");
                var start = NewStationButton("S" + station + " 启动");
                continuous.Click += async (s, e) => await ExecuteSafe(async () =>
                {
                    var address = PlcAddresses.StationContinuous(station);
                    var current = Convert.ToBoolean(await _plc.ReadAsync(address, _token));
        ...
        {
            var width = Math.Max(760, ClientSize.Width - 44);
        ...
        {
            var holder = new Panel { Dock = DockStyle.Fill, BackColor = Color.White };
