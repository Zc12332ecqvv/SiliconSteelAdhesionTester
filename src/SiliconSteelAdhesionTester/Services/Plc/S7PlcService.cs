            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    if (handler != null) handler(this, new PlcSnapshot
                    {
                        ...
                        WholeLineHome = Convert.ToBoolean(await _plc.ReadAsync(PlcAddresses.WholeLineHome, cancellationToken).ConfigureAwait(false)),
                        S1AutomaticRunning = Convert.ToBoolean(await _plc.ReadAsync(PlcAddresses.S1AutomaticRunning, cancellationToken).ConfigureAwait(false)),
                        S2HasPendingMaterial = Convert.ToBoolean(await _plc.ReadAsync(PlcAddresses.S2HasPendingMaterial, cancellationToken).ConfigureAwait(false)),
