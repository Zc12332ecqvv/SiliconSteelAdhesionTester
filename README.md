...
主界面包含由仿真/PLC快照驱动的八节点流程条：AGV送料、S1来料、扫码、条码校验、相机拍照、视觉分类、工位加工、等待来料。颜色约定为绿色已完成、蓝色执行中、白色未执行、黄色暂停、红色故障。
...
当前交互表没有给出条码字符串、视觉结果数值、产量和统一故障码的 DB 地址，因此首版没有虚构这些实体 PLC 点位；它们补齐后再接入生产业务服务。
